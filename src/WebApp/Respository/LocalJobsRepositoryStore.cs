...
public class LocalJobsRepositoryStore : IJobsRepository
{
    ...
    /// they do not.</remarks>
    /// <param name="options">Configuration options for the service, including paths for profile and job directories. Cannot be <see langword="null"/>.</param>
    ...
    /// <summary>
    /// Asynchronously retrieves the job profile for the specified job identifier.
    /// </summary>
    /// <remarks>This method attempts to read a job profile from a JSON file located in a predefined
    ...
    /// <param name="jobId">The unique identifier of the job whose profile is to be retrieved.</param>
    /// <returns>A <see cref="JobSummary"/> object containing the job details. If the job profile is not found, returns a default
    /// <see cref="JobSummary"/> with unknown values.</returns>
    /// <exception cref="InvalidDataException">Thrown if the deserialized job profile is null.</exception>
    ...
    public async Task<JobSummary?> GetJobsAsync(string jobId)
    {
        ...
        {
            _logger.LogError(ex, "Failed to load profile for job Id: {JobId}", jobId);
            throw new InvalidOperationException($"Failed to load job profile for ID: {jobId}", ex);
        }
    }
    ...
    public async Task<IEnumerable<JobSummary>> GetJobsAsync(DateTime fromDate)
    {
        ...
        // Fix: Return the sorted list
        return [.. jobSummaries.OrderByDescending(j => j.PostedDate)];
    }

    /// <summary>
    /// Asynchronously adds a job profile by saving it to a file in JSON format.
    /// </summary>
    /// <remarks>The job profile is saved in a directory specified by the job profile directory, with the
    /// filename based on the job's ID. If an error occurs during the save operation, the exception is logged and
    /// rethrown.</remarks>
    /// <param name="job">The job profile to be saved. Cannot be <see langword="null"/>.</param>
    ...
    public async Task AddJobAsync(JobSummary job)
    {
        ...
        {
            _logger.LogError(ex, "Failed to save job profile: {JobId}", job.JobId);
            throw;
        }
    }

    /// <summary>
    /// Asynchronously updates a job profile by saving it to a file in JSON format.
    /// </summary>
    /// <remarks>If the job profile doesn't exist, it will be created. The UpdatedAt timestamp is automatically set.</remarks>
    /// <param name="job">The job profile to be updated. Cannot be <see langword="null"/>.</param>
    /// <returns>A task representing the asynchronous operation.</returns>
    /// <exception cref="ArgumentNullException">Thrown when job is null.</exception>
    /// <exception cref="ArgumentException">Thrown when job ID is null or empty.</exception>
    public async Task UpdateJobAsync(JobSummary job)
    {
        ...
        try
        {
            if (!File.Exists(path))
            {
                _logger.LogInformation("Job profile not found, creating new: {JobId}", job.JobId);
                await AddJobAsync(job);
                return;
            }

            JobSummary? existingJob = null;

            // Read existing job
            using (FileStream readStream = File.OpenRead(path))
            {
                existingJob = await JsonSerializer.DeserializeAsync<JobSummary>(readStream, _options);
            }
            ...
            if (existingJob != null)
            {
                ...
                foreach (var property in properties)
                {
                    ...
                    property.SetValue(existingJob, incomingValue);
                }
                ...
                {
                    updatedAtProperty.SetValue(existingJob, DateTime.UtcNow);
                }
            }
            ...
            {
                await JsonSerializer.SerializeAsync(writeStream, existingJob ?? job, _options);
            }

            _logger.LogInformation("Job profile updated: {JobId}", job.JobId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to update job profile: {JobId}", job.JobId);
            throw;
        }
    }

    /// <summary>
    /// Asynchronously deletes the specified job profile from the storage directory.
    /// </summary>
    /// <param name="job">The job profile to delete. Cannot be <see langword="null"/>.</param>
    /// <returns>A task representing the asynchronous operation.</returns>
    /// <exception cref="ArgumentNullException">Thrown when job is null.</exception>
    /// <exception cref="ArgumentException">Thrown when job ID is null or empty.</exception>
    public async Task DeleteJobAsync(JobSummary job)
    {
        ...
        await Task.Run(() =>
        {
            try
            {
                ...
                {
                    File.Delete(path);
                    _logger.LogInformation("Job profile deleted: {JobId}", job.JobId);
                }
                else
                {
                    _logger.LogWarning("Job profile not found for deletion: {JobId}", job.JobId);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to delete job profile: {JobId}", job.JobId);
                throw;
            }
        });
    }
