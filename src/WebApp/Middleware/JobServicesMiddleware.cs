...
public sealed class JobServicesMiddleware(
RequestDelegate next,
IOptions<EzLeadSettings> options,
IJobSourceService jobSourceService,
ILogger<JobServicesMiddleware> logger)
{
    private readonly int _jobExecutionInSeconds = ValidateJobExecutionInSeconds(options);
    private readonly SemaphoreSlim _semaphore = new(1, 1);
    private readonly RequestDelegate _next = next ?? throw new ArgumentNullException(nameof(next));
    private readonly IJobSourceService _jobSourceService = jobSourceService ?? throw new ArgumentNullException(nameof(jobSourceService));
    private readonly ILogger<JobServicesMiddleware> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
